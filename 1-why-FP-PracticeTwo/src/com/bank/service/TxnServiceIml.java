...
//		};
		...
		return TxnUtilLib.filter(in, s-> s.getAmount()==amount);
...
//		}
	...
	/*	List<Txn> out = new ArrayList<Txn>();
		...
		return TxnUtilLib.filter(in, s->s.getLocalDateTime().toLocalDate().equals(date));
	}
...
//			}
	...
	public List<Txn> getTxns(LocalDateTime from, double amount) {
		List<Txn> in = txnRepository.findAll();
		return TxnUtilLib.filter(in, s->s.getLocalDateTime().equals(from));
	}
